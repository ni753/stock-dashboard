...
# --- Plotting Function ---
def plot_stock_chart(data, selected_symbol, is_intraday=True):
    if is_intraday:
        data.index = data.index.tz_convert('Asia/Kolkata')
        data['Time'] = data.index.strftime('%H:%M:%S')
    else:
        data['Time'] = data.index.strftime('%d-%b')
    ...
    latest_time = data.index[-1].strftime('%H:%M:%S') if is_intraday else data.index[-1].strftime('%d-%b')
    st.metric(label=f"📊 {selected_company} Current Price", value=f"₹ {latest_price:.2f}", delta=f"As of {latest_time} IST")

    st.subheader(f"{selected_symbol} - EMA Crossover Chart ({'Intraday' if is_intraday else 'Daily'})")
...
if selected_company:
    ...
    try:
        # First try intraday data (5-min)
        data = yf.download(selected_symbol, period="1d", interval="5m")
        if data.empty or "Close" not in data.columns:
            raise ValueError("Intraday data not available")
        # Intraday chart
        plot_stock_chart(data, selected_symbol, is_intraday=True)

    except Exception:
        try:
            # Fallback to daily data (2 previous days)
            data = yf.download(selected_symbol, period="2d", interval="1d")
            if data.empty or "Close" not in data.columns:
                raise ValueError("Daily data not available")
            plot_stock_chart(data, selected_symbol, is_intraday=False)
        except Exception:
            st.warning("⚠️ Stock data not available at the moment. Please try again later.")
else:
