...
# 🔥 Indian Market Live Data on Top
st.subheader("🇮🇳 Indian Market Live Indices")
...
# Create 2 columns for indices
col1, col2 = st.columns(2)

for idx, (name, symbol) in enumerate(indices.items()):
    try:
        index_data = yf.Ticker(symbol).history(period="1d", interval="1m")
        ...
        if idx == 0:
            with col1:
                st.metric(label=name, value=f"₹ {latest_price:.2f}", delta=f"{change:+.2f} ({change_percent:+.2f}%)")
        else:
            with col2:
                st.metric(label=name, value=f"₹ {latest_price:.2f}", delta=f"{change:+.2f} ({change_percent:+.2f}%)")
    except:
        if idx == 0:
            with col1:
                st.error(f"Failed to fetch {name} data.")
        else:
            with col2:
                st.error(f"Failed to fetch {name} data.")
